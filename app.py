...
daily_love_quotes = [
    ...
    "زیباترین قاب دنیا، لبخند بی‌دلیل و از ته دلیه که روی لبت می‌شینه. همیشه بخند 🌸",
    "فاصله‌ها روی نقشه هستن، ولی تو دقیقاً وسط قلب و فکر من زندگی می‌کنی ✨",
    "دنیا شلوغه و پر از هیاهو، ولی کنار تو امن‌ترین و قشنگ‌ترین نقطه جهان ساخته میشه 💫",
    "توی شلوغ‌ترین روزها هم کافیه بهت فکر کنم تا تمام خستگی‌هام پر بکشه و بره 🌻",
    "ممنونم که با اومدنت به زندگیم ثابت کردی مهربونی و صداقت هنوز واقعی‌ترین حس دنیاست 💖",
]

daily_motivation_quotes = [
    "امروز رو با انرژی شروع کن؛ تو از پس هر چالشی که جلوت قرار بگیره با درخشش برمیای! 💪✨",
    "هیچ‌وقت یادت نره که چقدر باارزش و قوی هستی، به خودت و توانایی‌هات ایمان داشته باش 🌟",
    "هر روز یک شروع تازه‌ست برای ساختن آرزوهات؛ قدم‌های کوچیکت آینده‌های بزرگ می‌سازن 🌱",
    "جهان به لبخند و انرژی مثبتت احتیاج داره، امروز بدرخش و اجازه نده چیزی خوشحالیت رو کم کنه ☀️",
    "تو قوی‌تر از تمام روزهای سختی هستی که پشت سر گذاشتی؛ امروز قراره یک روز فوق‌العاده باشه 🚀",
    "به رویاهات باور داشته باش، چون لیاقت بهترین‌ها دقیقاً متعلق به خودِ توئه 💎",
    "نفس عمیق بکش، لبخند بزن و با اعتماد به نفس به سمت هدفت برو؛ من همیشه باورت دارم 🤍",
]
...
with tabs[0]:
    ...
    st.markdown(
        "<p style='color:#94a3b8; font-size:12.5px; margin-top:2px;'>۱۷ آگوست ۲۰۲۶ | ساعت ۱۳:۰۶ (۱:۰۶ بعد از ظهر به وقت تهران)</p>",
        unsafe_allow_html=True,
    )
...
with tabs[1]:
    ...
    st.markdown(
        "<p style='color:#cbd5e1; font-size:12.5px;'>هر سوالی گوشه ذهنت داری بنویس تا با صداقت کامل بهت جواب بدم:</p>",
        unsafe_allow_html=True,
    )
...
with tabs[2]:
    ...
    positives = [
        ("منطقی بودن", "گفتگوی منطقی و آرام در تمام چالش‌ها اولویت اول منه."),
        ("صبوری و درک متقابل", "توی روزهای سخت و بی‌حوصلگی‌ها با آرامش کنارت می‌مونم."),
        ("وفاداری به پارتنر", "تعهد، تمرکز و وفاداری کامل روی رابطه‌مون."),
    ]
    for t, d in positives:
        st.markdown(
            f'<div class="flag-card green-flag"><strong>✔ {t}:</strong> {d}</div>',
            unsafe_allow_html=True,
        )
    ...
    for t, d in negatives:
        st.markdown(
            f'<div class="flag-card red-flag"><strong>✖ {t}:</strong> {d}</div>',
            unsafe_allow_html=True,
        )
...
with tabs[3]:
    st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    st.markdown(
        "<h4 style='color:#f472b6; margin:0;'>🕌 آزمون احکام و تقوا (نسخه طنز)</h4>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p style='color:#cbd5e1; font-size:12.5px;'>پاسخ به مسائل شرعی با چاشنی خنده 😂</p>",
        unsafe_allow_html=True,
    )
    ...
    if st.button("📿 محاسبه درجه تقوا", key="mbtn_deen"):
        if q1 and q2 and q3:
            st.success("نتیجه: تبارک‌الله! درجه تقوای شما در بالاترین سطح شوخ‌طبعی قرار دارد 😄🤍")
            st.snow()
        else:
...
with tabs[4]:
    ...
    if st.button("گشودن قفل 🗝️", key="mbtn_lock"):
        ...
        if (
            ...
            or ("بوس" in cleaned_pwd and "طلب" in cleaned_pwd)
        ):
            st.balloons()
